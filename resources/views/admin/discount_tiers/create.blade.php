...
@section('content')
    <div class="max-w-3xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        <h1 class="text-3xl font-bold text-slate-800 tracking-tight">Tambah Tingkat Diskon Baru</h1>
        ...
        <div class="mt-8 bg-white shadow-xl rounded-lg p-6 sm:p-8">
            <form action="{{ route('admin.products.discount-tiers.store', $product->id) }}" method="POST" class="space-y-6">
                @csrf
                <div>
                    <label for="min_quantity" class="block text-sm font-medium text-slate-700 mb-1">Kuantitas
                        Minimum</label>
                    <input type="number" name="min_quantity" id="min_quantity" value="{{ old('min_quantity') }}" required
